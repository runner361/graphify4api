...
| Product feature docs | a directory tree where each subdirectory directly holding `.md` files is a product feature — emits one `feature` node per such directory (`contains`/`subfeature_of` EXTRACTED edges), then LLM-adjudicated `implemented_by`/`uses_entity` INFERRED edges linking each feature to the `api_operation`/`inferred_entity` nodes above. Cross-lingual (Chinese feature name ↔ English path tokens via doc-body terms); degrades to name-match when no LLM backend is configured |
