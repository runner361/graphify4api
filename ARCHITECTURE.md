...
| `feature_link.py` | `generate_feature_nodes(extraction)`, `run_feature_linking(extraction, *, llm_call=None, top_n=20, cache_root=None)` | merged extraction (mutated in place) → from product-doc directory structure, emits `feature` nodes + `contains`/`subfeature_of` EXTRACTED edges; then LLM-adjudicated (or name-match degraded) `implemented_by`/`uses_entity` INFERRED edges linking features to the `api_operation`/`inferred_entity` nodes above |
